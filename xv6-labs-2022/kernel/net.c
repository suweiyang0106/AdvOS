...
//Allocates a packet buff, suwei add
//use user pa as driver space.
struct mbuf *
mbufalloc_suwei(unsigned int headroom,uint64 addr, int n)
{
  ...
  struct mbuf *m;
  va0 = PGROUNDUP(addr);
  pa0 = walkaddr(pagetable, va0);//va is continuous, but pa is continuous in a 4K
  //memset((void*)pa0, 0, PGSIZE);//suwei add
  if (headroom > MBUF_SIZE)
    return 0;
  //m = kalloc();//suwei comment
  m = (struct mbuf *)pa0;//suwei add
  if (m == 0)
    return 0;
  m->next = 0;
  m->head = (char *)m->buf + headroom;
  m->len = 0;
  //memset(m->buf, 0, sizeof(m->buf));//suwei comment
  return m;
}
