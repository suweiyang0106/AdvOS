...
#include "user/user.h"
#define improvedflag 1//suwei add
//
...
ping(uint16 sport, uint16 dport, int attempts)
{
  ...
  char *suweibuf=malloc(4096*3);//suwei: 3 page for finding 4K aligned va
  suweibuf = "test msg";
  #endif
  uint32 dst;
  ...
  #if improvedflag//suwei add
  printf("\n1 suweibuf addr:%d, %d, %s\n",&suweibuf, suweibuf,suweibuf);//suwei add
  if((fd = connect(dst, sport, dport,suweibuf)) < 0){
  #else
...
#if improvedflag//suwei add
  if((fd = connect(dst, 10000, 53,suweibuf)) < 0){
#else 
...
main(int argc, char *argv[])
{
  ...
  for (i = 0; i < 1; i++)
    ping(2000, dport, 1000);
  uint64 period = uptime()-timer1;//suwei add
  printf("OK\n");
  printf("write and read time period: %d\n",period);//suwei add
